...
function isRouterContract(abi) {
  const isPluginRouter = index.isFeatureEnabled(index.AbiSchema.parse(abi), "PluginRouter");
  const isBaseRouter = index.isFeatureEnabled(index.AbiSchema.parse(abi), "DynamicContract");
  ...
  async transactionError(error) {
    ...
    try {
      ...
      if (metadata?.metadata.sources) {
        sources = await index.fetchSourceFilesFromMetadata(metadata, this.storage);
      }
    } catch (err) {
      // no-op
    }
...
async function enginePrepareRequest(transaction, signer, provider, storage) {
  ...
  if (transaction.functionName === "approve" && transaction.functionArgs.length === 2) {
    ...
    const {
      message: permit,
      signature: sig
    } = await index.signEIP2612Permit(signer, transaction.to, transaction.from, spender, amount);
    const message = {
      to: transaction.to,
      owner: permit.owner,
    ...
    const forwarderAddress = index.CONTRACT_ADDRESSES[transaction.chainId]?.openzeppelinForwarder || (await index.computeForwarderAddress(provider, storage));
...
exports.biconomySendFunction = biconomySendFunction;
...
exports.defaultGaslessSendFunction = defaultGaslessSendFunction;
exports.defenderSendFunction = defenderSendFunction;
