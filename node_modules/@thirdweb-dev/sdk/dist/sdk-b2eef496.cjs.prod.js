...
class UserWallet {
  ...
  async sendRawTransaction(transactionRequest) {
    ...
    if (!hasGasData) {
      ...
      const defaultGas = await index.getDefaultGasOverrides(this.connection.getProvider());
...
async function getEncodedConstructorParamsForThirdwebContract(contractName, chainId, storage) {
  ...
  if (extendedMetadata?.routerType === "plugin" || extendedMetadata?.routerType === "dynamic") {
    const deploymentInfo = await deployWithThrowawayDeployer.getDeploymentInfo(publishUri, storage, provider, create2Factory, clientId, secretKey);
...
async function deployCreate2Factory(signer, options) {
  ...
  const commonFactoryExists = await index.isContractDeployed(index.COMMON_FACTORY, signer.provider);
  if (commonFactoryExists) {
    return index.COMMON_FACTORY;
  }
  const enforceEip155 = await index.isEIP155Enforced(signer.provider);
  ...
  const deploymentInfo = index.CUSTOM_GAS_FOR_CHAIN[networkId] ? index.getCreate2FactoryDeploymentInfo(chainId, {
    gasPrice: index.CUSTOM_GAS_FOR_CHAIN[networkId].gasPrice,
    gasLimit: index.CUSTOM_GAS_FOR_CHAIN[networkId].gasLimit
  }) : index.getCreate2FactoryDeploymentInfo(chainId, {});
  const factoryExists = await index.isContractDeployed(deploymentInfo.deployment, signer.provider);

  // deploy community factory if not already deployed
  if (!factoryExists) {
    const gasPrice = index.CUSTOM_GAS_FOR_CHAIN[networkId]?.gasPrice ? index.CUSTOM_GAS_FOR_CHAIN[networkId].gasPrice : 100 * 10 ** 9;
    const gasLimit = index.CUSTOM_GAS_FOR_CHAIN[networkId]?.gasLimit ? index.CUSTOM_GAS_FOR_CHAIN[networkId].gasLimit : 100000;
    invariant__default["default"](gasLimit, "gasLimit undefined for create2 factory deploy");
    invariant__default["default"](gasPrice, "gasPrice undefined for create2 factory deploy");

    // send balance to the keyless signer
    const valueToSend = ethers.BigNumber.from(gasPrice).mul(gasLimit);
...
async function getDeployArguments(contractType, metadata, contractURI, signer, storage) {
  ...
  if (!chainEnum) {
    const forwarder = await index.computeForwarderAddress(signer.provider, storage);
    trustedForwarders = [forwarder];
  } else {
    trustedForwarders = contractType === index.PackInitializer.contractType ? [] : getDefaultTrustedForwarders(chainId);
  }
...
class ContractFactory extends index.ContractWrapper {
  ...
  async getDeployArguments(contractType, metadata, contractURI) {
    let trustedForwarders = contractType === index.PackInitializer.contractType ? [] : await this.getDefaultTrustedForwarders();
...
class ThirdwebSDK extends index.RPCConnectionHandler {
  ...
  async getContractFromAbi(address, abi) {
    const [resolvedAddress, {
      SmartContract
    }] = await Promise.all([index.resolveAddress(address), Promise.resolve().then(function () { return require('./smart-contract-b455e51e.cjs.prod.js'); })]);
    if (this.contractCache.has(resolvedAddress)) {
      return this.contractCache.get(resolvedAddress);
    }
...
class ContractDeployer extends index.RPCConnectionHandler {
  ...
  deployViaAutoFactory = /* @__PURE__ */transactions.buildDeployTransactionFunction(async (publishMetadataUri, deployMetadata, signer, initializerFunction, paramValues, options) => {
    ...
    const deploymentInfo = await deployWithThrowawayDeployer.getDeploymentInfo(publishMetadataUri, this.storage, this.getProvider(), create2Factory, this.options.clientId, this.options.secretKey);
    ...
    await deployWithThrowawayDeployer.deployWithThrowawayDeployer(signer, transactionsForThrowawayDeployer, options);

    // send each transaction directly to Create2 factory
    // process txns one at a time
    for (const tx of transactionsforDirectDeploy) {
      try {
        await deployWithThrowawayDeployer.deployContractDeterministic(signer, tx, options);
      } catch (e) {
  ...
  async getTransactionsForDeploy(publishMetadataUri, options) {
    ...
    if (extendedMetadata && extendedMetadata.factoryDeploymentData && (extendedMetadata.isDeployableViaProxy || extendedMetadata.isDeployableViaFactory || extendedMetadata.deployType === "autoFactory") && !forceDirectDeploy) {
      ...
      if (!implementationAddress || extendedMetadata.deployType === "autoFactory") {
        ...
        const deploymentInfo = await deployWithThrowawayDeployer.getDeploymentInfo(publishMetadataUri, this.storage, this.getProvider(), create2FactoryAddress, this.options.clientId, this.options.secretKey);
        const transactionsToSend = deploymentInfo.filter(i => i.transaction.data && i.transaction.data.length > 0);
        const transactionsforDirectDeploy = transactionsToSend.filter(i => {
          return i.type !== "infra";
        }).map(i => i.transaction);
        ...
        const transactionBatches = deployWithThrowawayDeployer.createTransactionBatches(transactionsForThrowawayDeployer);
